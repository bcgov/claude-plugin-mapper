...
if [ "${dry_run}" = "true" ]; then
    python plugins/plugin-mapper/scripts/sync_with_inventory.py --cleanup-only --dry-run
else
    python plugins/plugin-mapper/scripts/sync_with_inventory.py --cleanup-only
fi
```
